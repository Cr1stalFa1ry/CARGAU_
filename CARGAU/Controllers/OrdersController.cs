...
using Application.Orders.Commands.AddServicesToOrder;
using Application.Orders.Commands.DeleteServicesFromOrder;
using Application.Orders.Queries.GetOrder;
using Application.Orders.Queries.GetOrders;
using Application.Orders.Queries.GetServicesByOrderId;
...
public class OrdersController : ControllerBase
{
    ...
    [HttpGet("{id:guid}/get-services")]
    [ProducesResponseType(StatusCodes.Status200OK)]
    [ProducesResponseType(StatusCodes.Status404NotFound)]
    [ProducesResponseType(StatusCodes.Status400BadRequest)]
    public async Task<IResult> GetServicesByOrderId(Guid id, CancellationToken cancellationToken)
    {
        var services = await _mediator.Send(new GetServicesByOrderIdQuery(id), cancellationToken);
    ...
    [HttpPatch("add-services")]
    [ProducesResponseType(StatusCodes.Status204NoContent)]
    [ProducesResponseType(StatusCodes.Status404NotFound)]
    [ProducesResponseType(StatusCodes.Status400BadRequest)]
    public async Task<IResult> AddServicesToOrder([FromBody] AddServicesToOrderCommand request, CancellationToken cancellationToken)
    {
    ...
    [ProducesResponseType(StatusCodes.Status404NotFound)]
    [ProducesResponseType(StatusCodes.Status400BadRequest)]
    public async Task<IResult> UpdateStatusOrder([FromBody] UpdateStatusOrderCommand request, CancellationToken cancellationToken)
    {
        return await _mediator.Send(request, cancellationToken)
            ? Results.NoContent() : Results.NotFound("order is not found");
    }

    [HttpDelete("{id:guid}/selected-services")]
    [ProducesResponseType(StatusCodes.Status204NoContent)]
    [ProducesResponseType(StatusCodes.Status404NotFound)]
    [ProducesResponseType(StatusCodes.Status400BadRequest)]
    public async Task<IResult> DeleteServicesFromOrder(Guid id, [FromBody] List<int> ServiceIdsToDelete, CancellationToken cancellationToken)
    {
        await _mediator.Send(new DeleteServicesFromOrderCommand(id, ServiceIdsToDelete), cancellationToken);
        return Results.NoContent();
    }
